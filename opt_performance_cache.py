...
def run_single_fio_test(mount_point: str, test_config: dict) -> dict:
    ...
    try:
        ...
        # Cleanup test files quickly
        run_command(f"rm -rf {test_dir}", ignore_errors=True)
...
def cleanup(md_device: str, vg_name: str, lv_name: str, mount_point: str):
    ...
    # 2. Kill processes and stop udisks2 (no sleep)
    run_command(f"fuser -km {md_device}", ignore_errors=True)
    run_command("killall md0_raid6", ignore_errors=True)
    run_command("systemctl stop udisks2", ignore_errors=True)
...
def full_setup():
    ...
    other_disks = [d for d in available if d['path'] not in data_disks]
    cache_disk = None

    if other_disks and confirm_action("Would you like to add a cache disk?"):
        ssd_disks = [d for d in other_disks if d.get('is_ssd', False)]
        if ssd_disks:
            print("\n[INFO] SSD cache disks available:")
            cache_selection = select_disks(ssd_disks, 1, "Cache Disk", exclude_list=data_disks)
        else:
            cache_selection = select_disks(other_disks, 1, "Cache Disk", exclude_list=data_disks)

        if cache_selection:
            cache_disk = cache_selection[0]
            print(f"[INFO] Cache disk: {cache_disk}")
